...
if __name__ == '__main__':
  ...
  args = parse_args()
  src_xml_dir = '/home/io/data/shiyu/pseudo'
  ...
  from model.faster_rcnn.vgg16_SARDA import vgg16
  from model.faster_rcnn.resnet_SARDA import resnet

  if args.net == 'vgg16':
      fasterRCNN = vgg16(imdb.classes, pretrained=True, class_agnostic=args.class_agnostic,
                         low_attention = args.low_ATT)
  else:
  ...
  for i in range(num_images):
      ...
      rois_label,d_pred,_,_ = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)
      ...
      for j in xrange(1, imdb.num_classes):
          ...
          if inds.numel() > 0:
            ...
            spt = cls_dets.cpu().numpy()
            #this is the part to produce xml file
            img = cv2.imread(imdb.image_path_at(i))
            width, height = img.shape[1],img.shape[0]
            xml_file = open((src_xml_dir + '/' + imdb.image_index[i] + '.xml'), 'w')
            xml_file.write('<annotation>\n')
            xml_file.write('    <folder>JPEGImages</folder>\n')
            xml_file.write('    <filename>' + str(imdb.image_index[i]) + '.png' + '</filename>\n')
            xml_file.write('    <size>\n')
            ...
            xml_file.write('    </size>\n')
            conf_av = 0
            for boxnumber in range(len(spt)):
              #spt = img_each_label.split(' ') #这里如果txt里面是以逗号‘，’隔开的，那么就改为spt = img_each_label.split(',')。
              conf_av += spt[boxnumber][4]
              ...
              xml_file.write('            <xmin>' + str(int(spt[boxnumber][0])) + '</xmin>\n')
              xml_file.write('            <ymin>' + str(int(spt[boxnumber][1])) + '</ymin>\n')
              xml_file.write('            <xmax>' + str(int(spt[boxnumber][2])) + '</xmax>\n')
              xml_file.write('            <ymax>' + str(int(spt[boxnumber][3])) + '</ymax>\n')
              xml_file.write('        </bndbox>\n')
              xml_file.write('    </object>\n')
            xml_file.write('</annotation>')
            conf_av = conf_av/len(spt)
            f = open('/home/io/data/shiyu/conf-av/conf_av.txt','a')
            f.write('\n'+str(conf_av))
            f.write('\n'+str(imdb.image_path_at(i)))
